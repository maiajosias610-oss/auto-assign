...
收尾日期对阈值**不保证单调**——换个阈值会把任务重排到不同人身上，结果可能倒挂
（实测过阈值 30 → 10-23 收尾、阈值 8 → 09-28 收尾）。所以引擎**不用二分**，
而是在 `[max_load_days_min, max_load_days_max]` 全区间扫描，**取收尾最早的那一档**
（收尾相同时取更小阈值 = 更均衡）。
全都压不进 deadline 就报警，同时给出收尾最早的一版供人工决策，不硬排。
